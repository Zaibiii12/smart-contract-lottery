...
contract RaffleTest is Test, CodeConstants {
    ...
    function testDontAllowPlayersToEntertWhileRaffleIsCalculating() public {
        vm.prank(PLAYER);
        raffle.enterRaffle{value: entranceFee}();
    ...
    function testFulfillRandomWordsPicksAWinnerResetsAndSendsMoney() public raffleEnteredS skipFork {
        ...
        uint256 winnerStartingBalance = address(expectedWinner).balance;
        vm.warp(startingTimeStamp + interval + 1);
        vm.roll(block.number + 1);
        vm.recordLogs();
        raffle.performUpkeep("");
        Vm.Log[] memory entries = vm.getRecordedLogs();
